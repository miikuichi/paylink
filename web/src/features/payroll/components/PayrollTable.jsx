...
const computeSssMonthly = (salary) => {
  ...
  const roundedBracket = Math.max(4500, Math.min(30000, Math.round(clipped / 500) * 500))
...
const estimatePreview = (basicRate, startDate, endDate, additionalItems) => {
  ...
  const taxMonthly = computeTaxMonthly(monthlyBasicRate, sssMonthly, philhealthMonthly, pagibigMonthly)
  ...
  const statutoryTotal = round2(statutory.reduce((sum, item) => sum + item.amount, 0))
...
export function PayrollTable({ employees, payrolls, payPeriod, processLoading, onProcess }) {
  ...
  const addItem = (itemType) => {
    setAdditionalItems((prev) => [...prev, { itemType, label: '', amount: '' }])
  }
    ...
    <>
      ...
      {previewEmployee && preview && (
        <div className="modal-overlay" onClick={closePreview}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ width: 640, maxWidth: '96vw' }}>
            <h3 style={{ margin: '0 0 8px' }}>
              Payroll Preview: {previewEmployee.firstName} {previewEmployee.lastName}
            </h3>
            <p style={{ margin: '0 0 16px', opacity: 0.7, fontSize: 13 }}>
              This preview follows the current PH payroll rules and final values are computed by the backend.
            </p>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16, fontSize: 13 }}>
              <div>Period days: <strong>{preview.days}</strong></div>
              <div>Proration ratio: <strong>{preview.ratio.toFixed(4)}</strong></div>
              <div>Basic pay (prorated): <strong>{currency(preview.basicPay)}</strong></div>
              <div>Gross pay estimate: <strong>{currency(preview.grossPay)}</strong></div>
            </div>
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: 8, padding: 10, marginBottom: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                <Button size="sm" variant="ghost" onClick={() => addItem('ALLOWANCE')}>Add Allowance</Button>
                <Button size="sm" variant="ghost" onClick={() => addItem('DEDUCTION')}>Add Deduction</Button>
              </div>
              ...
              {additionalItems.map((item, index) => (
                ...
                >
                  ...
                  <input
                    ...
                    style={{ padding: '6px 8px', borderRadius: 6, border: '1px solid #d1d5db' }}
                  />
                  <Button size="sm" variant="ghost" onClick={() => removeItem(index)}>Remove</Button>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <Button variant="ghost" size="sm" onClick={closePreview}>Cancel</Button>
              <Button size="sm" loading={processLoading} onClick={handleConfirmProcess}>Confirm & Process</Button>
            </div>
          </div>
        </div>
      )}
      ...
      <DataTable
        columns={[
          ...
          },
          {
            ...
            header: 'Breakdown',
            render: (r) => {
              const deductionItems = (r.items ?? []).filter((item) => item.itemType === 'DEDUCTION')
              ...
              return (
                <div style={{ display: 'grid', gap: 6 }}>
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => setExpandedPayrollId((prev) => (prev === r.id ? null : r.id))}
                  >
                    {isExpanded ? 'Hide' : 'Show'} Deductions
                  </Button>
                  {isExpanded && (
                    <div style={{ border: '1px solid #e5e7eb', borderRadius: 6, padding: '6px 8px', fontSize: 12 }}>
                      {deductionItems.length === 0 ? (
                        <span style={{ opacity: 0.7 }}>No deduction line items.</span>
                      ) : (
                        deductionItems.map((item) => (
                          <div key={item.id ?? item.label} style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
