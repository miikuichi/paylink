...
export function HrPayslipTable({ payslips, title, subtitle, onRevokePayslip }) {
...
export function EmployeePayslipTable({ payslips }) {
  ...
  const governmentDeductions =
    selectedPayslip?.deductions?.filter((item) =>
      GOVERNMENT_DEDUCTION_LABELS.has(item.label),
    ) ?? [];
  const otherDeductions =
    selectedPayslip?.deductions?.filter(
      (item) => !GOVERNMENT_DEDUCTION_LABELS.has(item.label),
    ) ?? [];
