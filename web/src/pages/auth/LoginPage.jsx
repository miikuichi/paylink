...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(user.role === 'ADMIN' ? '/hr/dashboard' : '/employee/dashboard', { replace: true })
    } catch (err) {
    ...
    >
      <form className="auth-form" onSubmit={handleSubmit} noValidate>
        ...
        <div className="auth-form__demo">
          <p className="auth-form__demo-title">Demo credentials (development only)</p>
          <div className="auth-form__demo-list">
            {MOCK_ACCOUNTS.map((acc) => (
              <button
                type="button"
                key={acc.username}
                className="auth-form__demo-chip"
                onClick={() => fillDemo(acc)}
              >
                <span className="auth-form__demo-role">{acc.role === 'ADMIN' ? 'HR / Admin' : 'Employee'}</span>
                <span className="auth-form__demo-cred">{acc.username} · {acc.password}</span>
              </button>
            ))}
          </div>
        </div>
